...
| `/cv` | Redirect to the original résumé PDF | Static redirect |
| `/cv.pdf` | Verbatim, user-approved résumé PDF | Static asset |
...
Each collapsed row contains company, role, dates, location, and a one-line contribution. Expanded content contains at most three bullets. The PDF CV retains the original résumé detail.
...
The user subsequently approved serving `Sky Lu Resume (14).pdf` verbatim as the
CV on September 6, 2026. Store its unchanged bytes at `public/cv.pdf`; `/cv`
redirects there. Do not create an HTML CV or regenerate the supplied PDF. Initial
...
On September 6, 2026, the user explicitly requested that `Sky Lu Resume (14).pdf`
be served verbatim. This supersedes the proposed web CV. Publish only that
approved original as `/cv.pdf`, preserving its layout, hyperlinks, and metadata;
`/cv` redirects to it. Any contact information in a PDF remains directly
extractable and is not covered by HTML email obfuscation. Review later replacement
documents independently and keep contact values out of other source and metadata.
...
- Preserve the GPA values in the explicitly approved PDF CV; do not put them in the hero.
- Confirm that company metrics remain approved for publication.
- Serve the approved `Sky Lu Resume (14).pdf` unchanged; review any later replacement.
