...
The supplied résumé PDF must not be published unchanged because its Brown email is extractable and bypasses HTML obfuscation. Keep that original outside the repository. `/cv` redirects to a separately reviewed public PDF with raw email addresses removed and a website/contact-form reference in their place.
...
The sanitized résumé is a small, versioned site asset. Store it at `public/sky-lu-resume.pdf` so Astro copies it into the same Cloudflare Workers Static Assets deployment as the rest of the site. `/cv` redirects to that stable asset URL. Do not introduce R2, KV, or a runtime binding for one PDF; reconsider R2 only if a larger media library or independent upload lifecycle appears.
...
- Produce and approve a sanitized public résumé PDF without raw email addresses.
