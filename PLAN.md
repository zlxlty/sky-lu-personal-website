...
Status: approved direction, Stage 03 content model in progress
...
| `/cv` | Redirect to the approved public résumé PDF | Static redirect |
...
- Small home icon aligned to the left rail.
...
Each collapsed row contains company, role, dates, location, and a one-line contribution. Expanded content contains at most three bullets. `/cv` opens the approved public résumé PDF rather than maintaining a second HTML résumé.
...
:root {
  ...
  --color-rule: rgb(56 51 47 / 10%);
  --color-rule-strong: rgb(56 51 47 / 25%);
...
[data-theme="dark"] {
  ...
  --color-rule: rgb(174 152 119 / 10%);
  --color-rule-strong: rgb(174 152 119 / 25%);
...
Content placement follows the narrowest useful boundary:

- Keep stable facts reused across sections or metadata in small, directly
...
- Keep project frontmatter limited to project-page and index metadata. Each
  project is a full-page MDX composition that imports its own layout and
  decorative components; `src/pages/project/[slug].astro` only resolves and
  renders the selected entry. Repetition between project files is acceptable
  when it preserves local control over panels, rule bands, separators, figures,
  and prose. Homepage-specific project copy remains in the homepage Astro
  template.
...
The approved public résumé is `Sky Lu Resume (15).pdf`, copied byte-for-byte to
`public/sky-lu-resume.pdf`. The approved file contains no extractable raw email
address. Every future replacement still requires a privacy review because email
addresses embedded in a PDF bypass Cloudflare's HTML obfuscation.
...
### 11.4 Contact write-endpoint protection

The planned contact form must:
...
Turnstile and rate limiting protect the form endpoint; they do not protect an address published elsewhere in HTML or a PDF. Recipient addresses remain server-side configuration and must never be returned to the browser.
...
The approved résumé is a small, versioned site asset. Store it verbatim at
`public/sky-lu-resume.pdf` so Astro copies it into the same Cloudflare Workers
Static Assets deployment as the rest of the site. `/cv` redirects to that stable
asset URL. Do not introduce R2, KV, or a runtime binding for one PDF; reconsider
R2 only if a larger media library or independent upload lifecycle appears.
...
- Confirm whether GPA values belong in the public résumé PDF; do not put them in the hero.
- Confirm that company metrics remain approved for publication.
- Re-review any replacement public résumé PDF for raw email addresses before
  publishing it verbatim.
