  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 grid max-h-[calc(100dvh-2rem)] w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 scale-100 scroll-p-[calc(var(--focus-width)+var(--focus-offset))] gap-4 overflow-y-auto overscroll-contain rounded-(--radius-medium) border border-(--color-rule-strong) bg-(--color-surface-raised) p-5 text-sm text-foreground shadow-[0_1rem_4rem_rgb(0_0_0/20%)] transition-[opacity,transform] duration-(--duration-disclosure) ease-(--ease-standard) outline-none data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0 motion-reduce:transition-none sm:max-w-md",
